...
function renameObjects(jsonData) {
    ...
    const renameArrayObjects = (array, newName) => {
      return array.map(item => {
        const newItem = {};
        newItem[newName] = item;
        return newItem;
      });
    };
  
    if (renamedData.nodes) {
      renamedData.nodes = renameArrayObjects(renamedData.nodes, 'node');
    }
  
    if (renamedData.vms) {
      renamedData.vms = renameArrayObjects(renamedData.vms, 'vm');
    }
  
    if (renamedData.containers) {
      renamedData.containers = renameArrayObjects(renamedData.containers, 'container');
    }
...
async function fetchVMsAndContainers() {
    ...
    function showNode(){
        ...
        nodes.forEach(node => {
            ...
            if(NodeElement == null){
                ...
                NodeElement.addEventListener("click", function(){showServer(node.node,node.node)});
        ...
        vms.forEach(vm => {
            if(vm.node==NodeNumber){
                ...
                if(ServerElement==null){
                    ...
                    <button type="button" class="float-right text-gray-900 bg-grey-200 border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
  ...
  function CheckStatus(vm) {
    if (vm.status === "running") {
        return `<button type="button" onclick="toggleVM(${vm.vmid}, 'stop', '${vm.node}')" class="float-right text-gray-900 bg-grey-200 border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
        ...
        return `<button type="button" onclick="toggleVM(${vm.vmid}, 'start', '${vm.node}')" class="float-right text-gray-900 bg-grey-200 border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
...
function toggleVM(vmid, action, node) {
    ...
    fetch(`/toggle_vm/${vmid}/${action}/${node}/`, {
        method: 'POST',
        headers: {
